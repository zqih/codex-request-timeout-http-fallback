...
## 方法二：保留 WebSocket，修好代理链路

适合仍然想使用 Codex 默认 WebSocket streaming 的情况。核心思路是：

- 恢复 Codex 默认 `openai` provider。
- 确认本地代理端口真的可用。
...
````markdown
请帮我修复 Codex 的 WebSocket 连接，不要降级 HTTP/SSE。目标是保留 Codex 默认 WebSocket streaming，并解决 `request timed out` 和自动重试 5 次。

你需要按下面步骤执行。改任何用户配置前都要先备份。不要打印代理订阅里的密码、token、节点密码或完整配置内容。
...
目标状态：
- `~/.codex/config.toml` 不再强制 `openai-http`
- Codex 继续使用内置 `openai` provider
...
2. 恢复 Codex 默认 WebSocket provider：

   打开 `~/.codex/config.toml`，删除或注释掉：
   ...
   注意：
   - 不要覆盖其他项目 trust 配置。
   - 删除后，Codex 会回到内置 `openai` provider。
...
5. 测试 WebSocket Upgrade 链路是否能到达远端。
